/**
 * VSS OTTB Dashboard - Full Implementation
 * Центр управления транками VSS AUTO HUB
 */
...
let terminalHistory = [];
let ottbApiUrl = 'http://localhost:8083';
...
// Загрузка транков из API
async function loadTrunksFromAPI() {
    try {
        const response = await fetch(`${ottbApiUrl}/api/trunks`);
        if (response.ok) {
        ...
        console.warn('[API] Не удалось загрузить транки из API, используем mock данные:', error);
    }
}
...
12:33:54 200 OK
        ...
        <div class="war-room-slot yellow">
            <div class="war-room-header">Slot ${slotIndex + 2} (MF) - DIALING</div>
            <div>Call: +380992345678 | Duration: 00:00:12</div>
            <div class="war-room-metrics">
                <div>MOS: --</div>
                <div>Jitter: --</div>
                <div>Loss: --</div>
            ...
            <div class="war-room-guac">GUACAMOLE: Mobile Device UI - Ringing</div>
            <pre class="war-room-pre">
12:34:00 INVITE
12:34:00 100 Trying
12:34:00 180 Ringing
12:34:12 ...RINGING...
            </pre>
        </div>
        
        <div class="war-room-slot green">
            <div class="war-room-header">Slot ${slotIndex + 3} (LS) - CONNECTED</div>
            <div>Call: +380993456789 | Duration: 00:01:45</div>
            <div class="war-room-metrics">
                <div>MOS: 3.8</div>
                <div>Jitter: 18ms</div>
                <div>Loss: 0.3%</div>
                <div>Latency: 52ms</div>
            </div>
            <div class="war-room-guac">GUACAMOLE: Windows RDP Session</div>
            <pre class="war-room-pre">
12:32:30 INVITE
12:32:30 100 Trying
12:32:30 180 Ringing
12:32:32 200 OK
12:32:32 ACK
12:32:32 RTP Stream Start
12:34:15 ...ACTIVE...
            </pre>
        </div>
    `;
...
function updateStats() {
    ...
    const activeTrunksEl = document.getElementById('activeTrunksCount');
    const totalSlotsEl = document.getElementById('totalSlots');
    const activeCallsEl = document.getElementById('activeCallsCount');
    
    if (activeTrunksEl) activeTrunksEl.textContent = `${activeTrunks}/${trunks.length}`;
    if (totalSlotsEl) totalSlotsEl.textContent = `${activeSlots}/${totalSlots}`;
    if (activeCallsEl) activeCallsEl.textContent = activeCallsCount;
}

// Обновление селектора быстрого вызова
...
async function addTrunk() {
    ...
    try {
        ...
        if (response.ok) {
            const newTrunk = await response.json();
            trunks.push(newTrunk);
            addLog(`Транк ${name} создан через API`);
        } else {
            throw new Error('API error');
        }
    } catch (error) {
        // Fallback: создание локально
        const newTrunk = {
            id: trunks.length + 1,
            name: name,
            tailscaleIp: address,
        ...
        trunks.push(newTrunk);
        addLog(`Транк ${name} создан локально`);
    }
...
function startLiveMonitoring() {
    setInterval(() => {
        ...
        if (cpuEl) cpuEl.textContent = Math.floor(Math.random() * 10 + 20) + '%';
        if (ramEl) ramEl.textContent = (Math.random() * 0.5 + 1.5).toFixed(1) + '/4GB';
        if (networkEl) networkEl.textContent = Math.floor(Math.random() * 30 + 100) + ' Mbps';
